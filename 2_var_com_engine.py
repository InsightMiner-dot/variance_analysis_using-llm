...
# ==========================================
# 6. STREAMLIT UI (SPLIT VIEW WITH SCROLL)
# ==========================================
...
if st.session_state.analysis_result is not None:
    ...
    if isinstance(summary_data, str):
        ...
        # Create Split View Columns (Left: Summary, Right: Trace)
        col_summary, col_trace = st.columns(2)
        
        # We define a fixed height for both scrollable containers
        CONTAINER_HEIGHT = 600
        
        with col_summary:
            st.markdown("### 🤖 Executive Summary")
            # Wrap the summary in a scrollable container
            with st.container(height=CONTAINER_HEIGHT):
                st.success(summary_data["ui_markdown_report"])
                
                with st.expander("⚙️ View Extracted JSON Structure"):
                    st.json(summary_data)
        
        with col_trace:
            st.markdown("### 🧮 Calculation Trace")
            # Wrap the calculation trace in a scrollable container of the exact same height
            with st.container(height=CONTAINER_HEIGHT):
